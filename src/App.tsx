...
const INITIAL_FIELDS = [
    ...
    { id: 'f4', name: 'Gender', type: 'select', label: 'Gender / Category', options: ['Der', 'Die', 'Das', 'Plural'] },
                ...
                < div className="space-y-3" >
                    ...
                    </div>
                    {
                        fields.map(field => (
                            ...
                            >
                                <div className={
                                    `w-4 h-4 rounded border mr-3 flex items-center justify-center ${template.front.includes(field.id) ? 'bg-indigo-500 border-indigo-500' : 'border-slate-300'
                                    }`
                                }>
                ...
                <div className="space-y-3" >
                    ...
                    </div>
                    {
                        fields.map(field => (
                            ...
                            >
                                <div className={
                                    `w-4 h-4 rounded border mr-3 flex items-center justify-center ${template.back.includes(field.id) ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300'
                                    }`
                                }>
...
const FieldRenderer = ({ field, value }) => {
    ...
    if (field.name === 'Gender') {
        const colors = { Der: 'bg-blue-100 text-blue-700', Die: 'bg-pink-100 text-pink-700', Das: 'bg-green-100 text-green-700', Plural: 'bg-yellow-100 text-yellow-700' };
        return (
            <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider shadow-sm ${colors[value] || 'bg-gray-100 text-gray-700'}`
            }>
                {value}
            </span>
        );
    ...
    if (field.name === 'Word') {
        return <h2 className="text-4xl sm:text-5xl font-extrabold text-slate-800 tracking-tight leading-tight text-center break-words w-full" > {value} </h2>;
    }
    ...
    if (field.name === 'Translation') {
        return (
            <div className="relative w-full text-center" >
                ...
                < p className="text-xl sm:text-2xl text-slate-600 font-serif italic break-words" > {value} </p>
            </div>
        );
    }
    ...
    if (field.name === 'Context') {
        return (
            <div className="w-full mt-2 bg-slate-50 p-4 rounded-xl border-l-4 border-indigo-400 text-left shadow-sm" >
                <p className="text-base sm:text-lg text-slate-700 leading-relaxed font-medium break-words" > "{value}" </p>
            </div>
        );
    }
...
const CardFaceContent = ({ fields, fieldIds, data, isBack, onAskAI, aiExplanation, isExplaining }) => {
    return (
        <div className={`w-full h-full bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 flex flex-col overflow-hidden [backface-visibility:hidden] ${isBack ? '[transform:rotateY(180deg)]' : ''}`
        }>
...
// 3. Study Mode
const StudySession = ({ deck, cards, fields, template, onExit }) => {
    ...
    if (finished) {
        return (
            <div className="flex flex-col items-center justify-center h-full space-y-6 animate-fade-in" >
                ...
                < button onClick={onExit} className="px-8 py-3 bg-slate-900 text-white font-medium rounded-xl hover:bg-slate-800 transition shadow-lg hover:shadow-xl translate-y-0 hover:-translate-y-0.5" >
                    Back to Dashboard
                </button>
            </div>
        );
    ...
    return (
        <div className="max-w-2xl mx-auto w-full h-full flex flex-col pb-8" >
            ...
            < div className="flex justify-between items-center mb-6 px-2" >
                <button onClick={onExit} className="text-slate-400 hover:text-slate-700 transition p-2 hover:bg-slate-100 rounded-full" >
                    <X className="w-5 h-5" />
                </button>
                < div className="flex gap-1.5" >
                    {
                        cards.map((_, idx) => (
                            <div key={idx} className={`h-1.5 rounded-full transition-all duration-300 ${idx === currentIndex ? 'w-8 bg-indigo-500' : idx < currentIndex ? 'w-2 bg-indigo-200' : 'w-2 bg-slate-200'}`} />
                        ))}
                </div>
            ...
            <div className="flex-1 perspective-1000 relative group cursor-pointer min-h-[400px]" onClick={() => !isFlipped && setIsFlipped(true)}>
                <div className={`grid grid-cols-1 grid-rows-1 w-full h-full transition-all duration-500 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
            ...
            {/* Controls */}
            <div className={`mt-8 grid grid-cols-4 gap-4 transition-all duration-500 ${isFlipped ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                <button onClick={() => handleGrade('again')} className="group p-4 rounded-2xl bg-white border border-slate-200 hover:border-red-200 hover:bg-red-50 transition-all shadow-sm hover:shadow-md" >
                ...
                < button onClick={() => handleGrade('hard')} className="group p-4 rounded-2xl bg-white border border-slate-200 hover:border-orange-200 hover:bg-orange-50 transition-all shadow-sm hover:shadow-md" >
                    <div className="text-orange-600 font-bold mb-1" > Hard </div>
                    < div className="text-xs text-slate-400 group-hover:text-orange-400 font-medium" > 10m </div>
                </button>
                < button onClick={() => handleGrade('good')} className="group p-4 rounded-2xl bg-white border border-slate-200 hover:border-green-200 hover:bg-green-50 transition-all shadow-sm hover:shadow-md" >
                ...
                < button onClick={() => handleGrade('easy')} className="group p-4 rounded-2xl bg-white border border-slate-200 hover:border-blue-200 hover:bg-blue-50 transition-all shadow-sm hover:shadow-md" >
                ...
                return (
                    <div className="max-w-4xl mx-auto" >
                        <div className="flex items-center mb-6" >
                            <button onClick={() => setView('dashboard')} className="mr-4 p-2 hover:bg-slate-100 rounded-full transition" > <ArrowLeft className="w-5 h-5 text-slate-600" /> </button>
                            < h1 className="text-2xl font-bold text-slate-800" > Deck Settings </h1>
                        </div>
                        < TemplateEditor fields={INITIAL_FIELDS} template={template} onUpdateTemplate={setTemplate} />

                        <div className="mt-8 flex justify-end" >
                            <button onClick={() => setView('dashboard')} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-2.5 rounded-lg font-medium transition shadow-sm" > Save Changes </button>
                        </div>
                    </div>
                );
                ...
                return (
                    <div className="max-w-2xl mx-auto" >
                        ...
                        < div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6 relative overflow-hidden" >
                            ...
                            < div className="absolute top-0 right-0 w-64 h-64 bg-indigo-50 rounded-full -translate-y-1/2 translate-x-1/2 opacity-50 pointer-events-none" > </div>

                            < div className="relative" >
                                <div className="flex justify-between items-end mb-2" >
                                    <label className="block text-sm font-semibold text-slate-700" > Word(Target Lang) </label>
                                    < button
                                ...
                                />
                                < p className="text-xs text-slate-400 mt-2 ml-1" >✨ Type a word and click "Auto-fill" to let AI generate the rest.</p>
                            </div>
                                        ...
                                        <label className="block text-sm font-semibold text-slate-700 mb-2" > {field.label} </label>
                                        {
                                            field.type === 'textarea' ? (
                                                <textarea
                                                    ...
                                                    onChange={(e) => setNewCardData({ ...newCardData, [field.id]: e.target.value })}
                                                />
                                            ) : field.type === 'select' ? (
                            ...
                            <button onClick={handleAddCard} className="w-full py-3.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition shadow-lg shadow-slate-200/50 mt-4" >
                                Add to Deck
                            </button>
                        </div>
                    </div>
                ...
                return (
                    <div className="max-w-4xl mx-auto" >
                        <header className="flex justify-between items-center mb-10" >
                            <div>
                                <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight" > Flashcard Forge </h1>
                            ...
                            < div className="flex gap-3" >
                                <button onClick={() => setView('editor')} className="flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 text-slate-700 font-medium rounded-lg hover:bg-slate-50 hover:border-slate-300 transition shadow-sm" >
                        ...
                        < div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" >
                            ...
                            < div className="bg-white group p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300" >
                                <div className="flex justify-between items-start mb-6" >
                                    ...
                                    < button onClick={() => setView('settings')} className="text-slate-300 hover:text-indigo-500 transition p-1 hover:bg-indigo-50 rounded-lg" >
                                ...
                                < div className="flex justify-between text-sm text-slate-500 mb-6 font-medium" >
                                    ...
                                    < span className="text-orange-500 bg-orange-50 px-2 py-0.5 rounded-md" > 15 due </span>
                                </div>

                                < div className="space-y-3" >
                                    <button onClick={() => setView('study')} className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl flex items-center justify-center gap-2 transition shadow-indigo-100 shadow-lg" >
